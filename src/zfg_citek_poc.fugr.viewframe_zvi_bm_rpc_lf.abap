*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_RPC_LF
*   generation date: 03.09.2019 at 11:38:57
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVI_BM_RPC_LF       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_LF'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_BM_RPC_LF_EXTRACT
TOTAL = ZVI_BM_RPC_LF_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_LF'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_RPC_LF-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_RPC_LF_EXTRACT
TOTAL = ZVI_BM_RPC_LF_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_RPC_LF-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_LF'
...
TOTAL = ZVI_BM_RPC_LF_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_RPC_LF-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_RPC_LF-UPD_FLAG NE SPACE.
