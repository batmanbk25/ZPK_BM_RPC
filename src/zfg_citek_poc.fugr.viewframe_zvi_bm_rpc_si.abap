*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_RPC_SI
*   generation date: 22.07.2019 at 15:42:36
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVI_BM_RPC_SI       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_SI'
         EXPORTING
              FCODE          = READ
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_SI'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_RPC_SI-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_RPC_SI_EXTRACT
TOTAL = ZVI_BM_RPC_SI_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_RPC_SI-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_RPC_SI'
...
TOTAL = ZVI_BM_RPC_SI_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_RPC_SI-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_RPC_SI-UPD_FLAG NE SPACE.
